...
def main():
    ...
    for season in STAT_SEASONS:
        ...
        season_stats[season] = fetch_json(
            f"{SLEEPER}/stats/nfl/regular/{season}", optional=True)
        print(f"  {len(season_stats[season])} lines")
    ...
    for rank, player in enumerate(players):
        ...
        seasons = {}
        for season in STAT_SEASONS:
            line = season_stats.get(season, {}).get(player_id)
            if line:
                block = compact(line)
                block["gp"] = int(line.get("gp") or 0)
