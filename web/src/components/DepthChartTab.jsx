...
export default function DepthChartTab({ engine, player }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {groups.map((g) => (
        <div key={g.group}>
          <p className="mb-1.5 text-[10px] font-semibold uppercase tracking-wide text-white/35">{g.group}</p>
            ...
            {g.players.map((p) => (
              <div
                ...
                className={
                  ...
                  (p.isSelf ? 'border-teal-400/40 bg-teal-500/10' : 'border-slate-800 bg-slate-950/50')
                }
              >
                <span className="w-4 shrink-0 text-center text-[10px] text-white/30">{p.order || '–'}</span>
                ...
                <span className="shrink-0 text-xs text-white/30">ADP {p.adp.toFixed(1)}</span>
              </div>
            ))}
          </div>
        </div>
