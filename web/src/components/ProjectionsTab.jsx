      ...
      {summary.stats.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-3 gap-1.5 lg:gap-2">
            {summary.stats.map((s) => (
              <div key={s.label} className="rounded-lg border border-slate-rule bg-slate-sunk/50 px-2 py-1.5 lg:px-3 lg:py-2.5 text-center">
                <p className="text-[10px] lg:text-xs font-medium uppercase tracking-wide text-ink-muted">{s.label}</p>
                <p className="text-sm lg:text-base font-semibold text-white">{s.value}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            <table className="w-full bg-slate-panel text-xs lg:text-sm">
              <thead>
                <tr className="border-b border-slate-rule bg-slate-sunk/60 text-[10px] lg:text-xs uppercase tracking-wide text-ink-muted">
                  ...
                  <th className="px-2 py-1.5 lg:px-3 lg:py-2.5 text-right font-medium">We said</th>
                  <th className="px-2 py-1.5 lg:px-3 lg:py-2.5 text-right font-medium">He got</th>
                  <th className="px-2 py-1.5 lg:px-3 lg:py-2.5 text-right font-medium">Diff</th>
              ...
              <tbody>
                {record.map((r) => (
                  <tr key={r.year} className="border-b border-slate-rule/60 last:border-b-0">
                    ...
                    <td className="px-2 py-1.5 lg:px-3 lg:py-2.5 text-right tabular-nums text-white/70">{Math.round(r.proj)}</td>
                    <td className="px-2 py-1.5 lg:px-3 lg:py-2.5 text-right tabular-nums text-white">{Math.round(r.act)}</td>
                    <td className={'px-2 py-1.5 lg:px-3 lg:py-2.5 text-right font-semibold tabular-nums ' +
