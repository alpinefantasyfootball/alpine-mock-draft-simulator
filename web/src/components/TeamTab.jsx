...
function SlotRow({ label, player }) {
  return (
    <div className="flex items-center gap-2.5 rounded-lg border border-slate-rule bg-slate-sunk/50 px-3 py-2">
      <span className={'w-11 shrink-0 rounded px-1.5 py-0.5 text-center text-[10px] font-bold ' + (player ? POS_BADGE[player.pos] || 'bg-white/10 text-white/50' : 'bg-white/5 text-ink-muted')}>
        {label}
      </span>
      {player ? (
        <span className="min-w-0 flex-1 truncate text-sm font-medium text-white/90">{player.name}</span>
      ) : (
        <span className="min-w-0 flex-1 text-sm text-ink-muted">Empty</span>
      )}
    </div>
  )
}
      ...
      {compact ? (
        <div className="shrink-0 p-2 pb-1.5">
          <select
            ...
            className="w-full rounded-lg border border-slate-rule bg-slate-sunk/60 px-2 py-1.5 text-xs font-semibold text-white outline-none focus:border-teal-400/60"
          >
